...
double color_similarity(cv::Mat image, cv::Rect _ri, cv::Rect _rj) {
    ...
    for (auto r : regions) {
        ...
        normalize(b_hist, b_hist, 0, 256, cv::NORM_MINMAX, -1, cv::Mat());
        normalize(g_hist, g_hist, 0, 256, cv::NORM_MINMAX, -1, cv::Mat());
        normalize(r_hist, r_hist, 0, 256, cv::NORM_MINMAX, -1, cv::Mat());
